...
- **修复网络故障后脚本不自愈的死循环**：走代理时（httpx HTTP/2 长连接），节点断连后连接池里的连接半死，脚本反复复用坏连接导致接口秒失败，即使网络恢复也一直卡在「运行异常」直到重启。现在每次重试前自动重建 API 客户端丢弃坏连接，网络恢复后自动推送「运行恢复」通知
- **系统设置新增「代理地址」**：主菜单可直接填写代理（如 `http://127.0.0.1:7890`），对应 `config.json` 的 `runtime.proxy`，大陆用户走代理连 OKX 不用再改配置文件
- **异常通知冷却**：运行异常同类 60 分钟内只推一次，不再反复轰炸
